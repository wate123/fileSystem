...
int main(int argc, char *argv[]) {
  ...
  f=create_file("superfile", READ_WRITE);
  printf("ret from create_file(\"superfile\", READ_WRITE) = %p\n",
	 f);
  fs_print_error();
  ...
  f=create_file("superfile-max", READ_WRITE);
  printf("ret from create_file(\"superfile-max\", READ_WRITE) = %p\n",
	 f);
  fs_print_error();
  buf=malloc(maxlen+1);
